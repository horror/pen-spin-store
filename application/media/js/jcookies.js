...
(function($){
	...
	// set raw_data for get or erase
		...
		// get cookie(s)
				...
				if(o.get == "*" && raw_data) {
					for(name in raw_data)
						try{
							raw_data[name] = JSON.parse(atob(decodeURI(raw_data[name])));
						}catch(exception){
							try{
								raw_data[name] = JSON.parse(atob(decodeURI(raw_data[name])));
							}catch(exception2){
			...
			// get single cookies	
				for(x in names)
					if(names[x] == o.get)
					try{
						return JSON.parse(atob(decodeURI(raw_data[o.get])));
					}catch(exception){
						if(o.error) return exception;
					}
	...
	// make cookie(s)
			...
			document.cookie = o.name + "=" + encodeURI(btoa(JSON.stringify(o.value))) + "; expires=" + date.toGMTString() + "; path=/";
			return true;
	// get cookie			
		} else return false;			
	}
