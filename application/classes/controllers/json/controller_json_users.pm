package controller_json_users; {
	...
	$self->data->{rows} = model_users->new($self->database_handler())
	    ->get_users_jbgrid_format_calls({ "-$sord" => $sidx }, $limit, $start);
	my $count = model_users->new($self->database_handler())->get_users_count();
...
=cut    
	...
	my $user_info = validation->new($self->request())->validate_user_card_form();   
        $user_info = {} unless ref $user_info eq "HASH";
	
	model_users->new($self->database_handler())->add_user($user_info);
    }
